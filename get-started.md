...
:::note
The Go client doesn't support [multi-tenancy](https://docs.camunda.io/docs/self-managed/concepts/multi-tenancy.md) and can only be used when multi-tenancy is disabled.
:::
...
2. Click on the link to [Operate](https://docs.camunda.io/docs/components/operate/userguide/basic-operate-navigation.md).
...
6. Additionally, for the service task `Collect Money` set a [**task-header**](https://docs.camunda.io/docs/components/modeler/bpmn/service-tasks/service-tasks.md#task-headers) with the key `method` and the value `VISA`. This header is used as a configuration parameter for the payment-service worker to hand over the payment method.
...
In this example, we open a [job worker](https://docs.camunda.io/docs/components/concepts/job-workers.md) for jobs of type `payment-service`.
...
- Learn more about the [concepts behind Zeebe](https://docs.camunda.io/docs/components/concepts/what-is-camunda-8.md).
- Learn more about [BPMN processes](https://docs.camunda.io/docs/components/modeler/bpmn/bpmn-primer.md).
